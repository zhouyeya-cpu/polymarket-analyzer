...
def call_openai_compatible(api_key, base_url, model, messages, temperature=0.15, max_tokens=7000):
...
def call_claude(api_key, messages, model="claude-opus-4-6", temperature=0.15, max_tokens=7000):
...
<script>
  ...
  const PROVIDER_HINTS = {
    ...
    claude:   { key: 'Anthropic API Key（console.anthropic.com）', model: 'claude-opus-4-6' },
...
def analyze():
    ...
    if not ai_key or not news_token or not raw_slug:
        return jsonify({"error": "缺少必要参数：ai_key / news_token / event_slug"}), 400
    ...
    try:
        ...
        # 5. 新闻
        try:
            news = get_news(news_token, window=news_window, q=news_query or "market event")
            news_text = format_news_for_prompt(news)
        except Exception as e:
            news_text = f"新闻获取失败：{e}"

        # 6. 构建 Prompt
        messages = build_prompts(pm_data, orderbook_summary, price_history_summary,
